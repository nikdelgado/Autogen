...
namespace {{ns_tpl}}
{
    bool Convert{{type_name}}::from_protobuf({{type_name | member.ns_override}}{{type_name}}& dest, const common::api::abb_protobuf::types::{{type_name}}& src)
    {
        ...
        switch (src.choice())
        {
            ...
            case common::api::abb_protobuf::types::{{type_name}}ChoiceType::{{type_name}}Choice_{{choice.name}}: 
            {
                {%-if choice.as_attr is member.is_native and choice.as_attr is not member.is_list %}
                dest.set{{choice.name}}(src.{{choice.name}}());
				break;
                {%- else %}
                auto temp_{{choice.raw_type}} = {{choice.raw_type | member.ns_override}}{{choice.raw_type}}();
    ...
    bool Convert{{type_name}}::to_protobuf(common::api::abb_protobuf::types::{{type_name}}& dest, const {{type_name | member.ns_override}}{{type_name}}& src)
    {
        ...
        {%- for choice in type_info|variant.choices %}
        if (std::holds_alternative<{{choice.raw_type | member.ns_override}}{{choice.raw_type}}>(src))
        {
            dest.set_choice(common::api::abb_protobuf::types::{{type_name}}ChoiceType::{{type_name}}Choice_{{choice.name}});
            {%- if choice.as_attr is member.is_native and choice.as_attr is not member.is_list %}
            dest.{{choice.name}}(src);
            break;
            {%- else %}
            auto temp_src = std::get<{{choice.raw_type | member.ns_override}}{{choice.raw_type}}>(src);
            using ConversionType = Converter<
