...
class AgFilters:
    """Defines custom jinja filters specific to classes generated by NCI
    ...
    def register(self, env: Environment):
        env.filters.update({
            ...
            "alias.accessor_type": self.alias_accessor,
            ...
            "uci.accessor_type": self.uci_accessor,
        ...
        env.tests.update({
            ...
            "uci.direct_get_set_opt": self.uci_direct_get_set,
            "message.root": self.is_root_class,
            "message.name_root": self.is_root_class_name,
    ...
    def is_root_class(self, clazz: Class) -> bool:
        """Specific to UCI, true if the class is the "root" message type
        class, which implies that it is sent over the ASB.
        """
        return self.is_root_class_name(clazz.name)

    def is_root_class_name(self, name: str) -> bool:
        """Specific to UCI, true if the class name is for a "root" message type
        class, which implies that it is sent over the ASB.
        """
        return name.endswith('MT')
    ...
    def uci_direct_get_set(self, attr: Attr) -> bool:
        """return true if a member stores the same value in REFCAL as in api/uci
        or the two types support implicit conversion.
        
        NOTE: if true this implies that you can call set with the return of get directly.
        """
        clazz = self.member_class(attr)
        return (
            self.is_simple_attr(attr) and 
            not self.alias_is_string(clazz)
        ) or self.is_custom_attr(attr)
    ...
    def alias_accessor(self, attr: Class) -> str:
        """This is a uci specific suffix for alias types, they put the name
        of the type in a different format in their typed getters.
        """
        prim = self.alias_primitive(attr)
        if "string" in prim:
            return "String"
        if "double" in prim:
            return "Double"
        ...
        if "int64" in prim:
            return "Long"
        if "speed" in prim.lower():
            # NOTE: I'm not sure if this one is necessary
            return "Double"
        raise Exception(f'UCI Alias does not currently support {attr.name}')
    ...
    def enum_name(self, attr: Attr) -> str:
        """return the keyword name of an enum. In the case of the uci
        namespace prefix with UCI_ because they have some c++ incompatible names (like 25X1 for classification)
        """
        return ('UCI_' if 'uci' in self.render_vars['ns_api'].lower() else '') + f'{attr.name}'
    ...
    def enum_value(self, attr: Attr) -> str:
        ...
        """
        # TODO(lt): add support for custom values/types
        return None
    ...
    def cls_abstract_base_type(self, clazz: Class) -> str:
        ...
        This is used in the OMS in at least one case where abstract classes
        are grouped, then subgrouped.
        """
    ...
    def uci_accessor(self, attr: Union[Class, Attr]) -> str:
        """generate the uci/xs accessor type enum value for a class/member.

        This is used for polymorphic/abstract members in REFCAL to specify which
        type is stored in a list/pointer.
        """
        if isinstance(attr, Class):
            clazz = attr
        else:
            clazz = self.member_class(attr)
        type_name = DataType.from_qname(clazz.qname) or clazz.name
        prim = self.alias_primitive(clazz) or ''
        if 'string' in type_name or 'string' in prim:
            return '::xs::accessorType::string'
        return f'::uci::type::accessorType::{self.__first_lower(clazz.name)}'
    ...
    def val_name(self, name: str) -> str:
        ...
        it might need some more items added but the OMS 2.1 spec is covered.
        """
